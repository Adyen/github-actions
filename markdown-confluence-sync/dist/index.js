...
var isObject = __nccwpck_require__(605);
...
const sections = __nccwpck_require__(364);
const defaults = __nccwpck_require__(802);
const stringify = __nccwpck_require__(259);
const excerpt = __nccwpck_require__(571);
const engines = __nccwpck_require__(649);
const toFile = __nccwpck_require__(296);
const parse = __nccwpck_require__(885);
const utils = __nccwpck_require__(119);
...
const yaml = __nccwpck_require__(58);
...
const getEngine = __nccwpck_require__(474);
const defaults = __nccwpck_require__(802);
...
const typeOf = __nccwpck_require__(190);
const getEngine = __nccwpck_require__(474);
const defaults = __nccwpck_require__(802);
...
const stripBom = __nccwpck_require__(708);
const typeOf = __nccwpck_require__(190);
...
var yaml = __nccwpck_require__(827);
...
var loader = __nccwpck_require__(857);
var dumper = __nccwpck_require__(887);
...
module.exports.Type = __nccwpck_require__(466);
module.exports.Schema = __nccwpck_require__(949);
module.exports.FAILSAFE_SCHEMA = __nccwpck_require__(233);
module.exports.JSON_SCHEMA = __nccwpck_require__(30);
module.exports.CORE_SCHEMA = __nccwpck_require__(747);
module.exports.DEFAULT_SAFE_SCHEMA = __nccwpck_require__(583);
module.exports.DEFAULT_FULL_SCHEMA = __nccwpck_require__(3);
...
module.exports.YAMLException = __nccwpck_require__(149);

// Deprecated schema names from JS-YAML 2.0.x
module.exports.MINIMAL_SCHEMA = __nccwpck_require__(233);
module.exports.SAFE_SCHEMA = __nccwpck_require__(583);
module.exports.DEFAULT_SCHEMA = __nccwpck_require__(3);
...
var common              = __nccwpck_require__(495);
var YAMLException       = __nccwpck_require__(149);
var DEFAULT_FULL_SCHEMA = __nccwpck_require__(3);
var DEFAULT_SAFE_SCHEMA = __nccwpck_require__(583);
...
var common              = __nccwpck_require__(495);
var YAMLException       = __nccwpck_require__(149);
var Mark                = __nccwpck_require__(75);
var DEFAULT_SAFE_SCHEMA = __nccwpck_require__(583);
var DEFAULT_FULL_SCHEMA = __nccwpck_require__(3);
...
var common        = __nccwpck_require__(495);
var YAMLException = __nccwpck_require__(149);
var Type          = __nccwpck_require__(466);
...
var Schema = __nccwpck_require__(949);
...
var typeOf = __nccwpck_require__(190);
var extend = __nccwpck_require__(566);
...
;// CONCATENATED MODULE: ./src/confluence-client.ts
...
const updateConfluencePage = async (pageId, { storage, title }) => {
...
var gray_matter = __nccwpck_require__(250);
var gray_matter_default = /*#__PURE__*/__nccwpck_require__.n(gray_matter);
;// CONCATENATED MODULE: ./src/documents.ts
...
const getRequiredMetadata = (data, field, source) => {
...
const findTrackedMarkdownFiles = async (repositoryRoot) => {
    ...
    return stdout
        .split('\0')
        .filter(Boolean)
        .map((filePath) => external_node_path_default().resolve(repositoryRoot, filePath))
        .sort();
};
/**
 * Find only markdown with confluence frontmatter
 */
const discoverConfluenceDocuments = async (repositoryRoot) => {
    const markdownFiles = await findTrackedMarkdownFiles(repositoryRoot);
    const documents = await Promise.all(markdownFiles.map(async (sourcePath) => {
        const markdown = await (0,promises_namespaceObject.readFile)(sourcePath, 'utf8');
        const frontmatter = gray_matter_default()(markdown);
        if (!('confluence_page_id' in frontmatter.data)) {
            return undefined;
        }
        const source = external_node_path_default().relative(repositoryRoot, sourcePath);
...
const markdownInlineToStorage = (markdown) => {
...
const markdownToConfluenceStorage = (markdown) => {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        if (heading) {
            output.push(`<h${heading[1].length}>${markdownInlineToStorage(heading[2])}</h${heading[1].length}>`);
            continue;
        }
...
const DEFAULT_BANNER = '<ac:structured-macro ac:name="warning"><ac:rich-text-body><p><strong>Auto-generated from GitHub. Do not manually edit.</strong>';
const syncDocumentationToConfluence = async () => {
    ...
    await Promise.all(documents.map(async (document) => {
        ...
        const banner = process.env.INPUT_BANNER ||
        ...
        try {
            ...
            throw new Error(`Failed to sync from "${document.source}" to Confluence page ${document.pageId}: ${message}`);
        }
    }));
};
syncDocumentationToConfluence().catch((error) => {
